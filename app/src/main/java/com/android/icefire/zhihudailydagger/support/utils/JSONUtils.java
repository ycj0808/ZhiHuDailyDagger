...
public class JSONUtils {
	...
	public static <T> T fromJson(String json, TypeToken<T> token, String datePattern) {
		...
		try {
			...
			Logger.e(json + " 无法转换为 " + token.getRawType().getName() + " 对象!---->%s", ExceptionUtils.getErrorInfo(ex));
			return null;
		}
	}
	/**
	...
	public static <T> T fromJson(String json, Class<T> clazz, String datePattern) {
		...
		try {
			...
			Logger.e(json + " 无法转换为 " + clazz.getName() + " 对象!--->%s", ExceptionUtils.getErrorInfo(ex));
			return null;
		}
	}
	/**
	...
	public static String toJson(Object target, Type targetType, GsonBuilder builder) {
		...
		try {
			...
			Logger.w("目标对象 " + target.getClass().getName() + " 转换 JSON 字符串时，发生异常！--->%s", ExceptionUtils.getErrorInfo(ex));
